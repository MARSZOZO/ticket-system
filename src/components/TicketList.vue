<template>
	<button type="button" class="btn btn-secondary mb-4 mt-3 float-end" data-bs-toggle="modal" data-bs-target="#newTicketModal">Новое обращение</button>
      ...
      <tr v-for="(ticket, index) in dataTicket" :key="index">
        ...
        <td>
					<div class="ticket-message">
						<ul>
							<li><span class="text-secondary">Тип обращения: </span><span class="text-info fw-bold">{{ticket.subject}}</span></li>
							<li><span class="text-secondary">{{ticket.body_subject}}</span></li>
						</ul>
					</div>
				</td>
